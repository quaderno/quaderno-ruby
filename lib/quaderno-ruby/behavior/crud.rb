  ...
  module Behavior
    module Crud         
      ...
      module ClassMethods
        ...
        def all(filter = nil)
          party_response = get("#{api_model.base_url}/#{ api_model.subdomain }/api/v1/#{ api_model.api_path }.json", body: filter, basic_auth: { username: api_model.auth_token })
        ...
        def find(id)
          party_response = get "#{api_model.base_url}/#{ api_model.subdomain }/api/v1/#{ api_model.api_path }/#{ id }.json", basic_auth: { username: api_model.auth_token }
        ...
        def create(params)
          party_response = post "#{api_model.base_url}/#{ api_model.subdomain }/api/v1/#{ api_model.api_path }.json", body: params, basic_auth: { username: api_model.auth_token }
        ...
        def update(id, params)
          party_response = put "#{api_model.base_url}/#{ api_model.subdomain }/api/v1/#{ api_model.api_path }/#{ id }.json", body: params, basic_auth: { username: api_model.auth_token }
          check_exception_for(party_response, { rate_limit: true, subdomain_or_token: true, id: true })
          hash = JSON::parse party_response.body                        
          if (api_model == Quaderno::Invoice) || (api_model == Quaderno::Estimate) || (api_model == Quaderno::Expense)
            api_model.parse(hash)
        ...
        def delete(id)
          party_response = HTTParty.delete "#{api_model.base_url}/#{ api_model.subdomain }/api/v1/#{ api_model.api_path }/#{ id }.json", basic_auth: { username: api_model.auth_token }
